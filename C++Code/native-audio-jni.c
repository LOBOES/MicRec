...
int Java_com_example_nativeaudio_NativeAudio_getLastVolume(JNIEnv* env, jclass clazz)
{
    if (recorderSize >= (RECORDER_FRAMES * sizeof(short))) {
        return 0;
    }
    for (int i = RECORDER_FRAMES-1; i >= 0; i--)
    {
        if (recorderBuffer[i] != 0)
        {
            return (recorderBuffer[i] > 0 ? 1 : -1) * recorderBuffer[i];
        }
    }
}
